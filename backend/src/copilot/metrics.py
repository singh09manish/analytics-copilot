...
which is why the ECS *task* role's only grant is the narrow, deliberate ECS Exec
exception (`ssmmessages:*`, see `infra/iam.tf`), not a metrics-related one. The task
role holds no permissions the application itself uses.
